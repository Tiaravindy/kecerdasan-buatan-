...
# Ambil fitur (embedding) semua waifu
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        # === Tambahan: cari waifu terbaik kalau wajahnya 'saya'
        if label == 'saya' and confidence >= 60:
            best_husbando, sim_score = get_best_husbando_match(img_array)
            cv2.putText(frame, f"Cocok dengan {best_husbando} ({sim_score:.2%})", (x, y+h+20),
